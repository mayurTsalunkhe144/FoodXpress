...
function RestaurantOrders() {
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      <div>
        <h1 className="text-4xl font-black mb-2" style={{ color: 'var(--text-primary)' }}>Restaurant Orders</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Total: {orders.length} orders</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {orders.map((order) => (
            <Card key={order.orderId} style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }} className="border">
              <CardContent className="pt-6">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p style={{ color: 'var(--text-primary)' }} className="text-sm">{new Date(order.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
