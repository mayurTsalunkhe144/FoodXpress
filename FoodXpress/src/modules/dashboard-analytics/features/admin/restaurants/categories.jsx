...
function RestaurantCategories() {
  ...
  return (
    <div className="space-y-8 pb-8">
      ...
      <div>
        <h1 className="text-4xl font-black mb-2" style={{ color: 'var(--text-primary)' }}>Menu Categories</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Total: {categories.length} categories</p>
      </div>

      {loading ? (
        <div className="text-center py-12" style={{ color: 'var(--text-secondary)' }}>Loading categories...</div>
      ) : categories.length === 0 ? (
        <div className="text-center py-12" style={{ color: 'var(--text-secondary)' }}>No categories found</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((category) => (
            <Card key={category.categoryId} style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }} className="border">
              <CardContent className="pt-6">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p style={{ color: 'var(--text-primary)' }} className="text-sm">{category.description || 'No description'}</p>
                  </div>
                  <div>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Items Count</p>
                    <p style={{ color: 'var(--text-primary)' }} className="font-semibold">{category.itemCount || 0} items</p>
                  </div>
                </div>
              </CardContent>
            </Card>
